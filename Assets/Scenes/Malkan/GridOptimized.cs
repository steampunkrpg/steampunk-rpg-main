...
public class GridOptimized : MonoBehaviour
{
    ...
    void OptimizeTiles()
    {
        for (int i = 0; i < tileList.Count; i++)
        {
            ...
            Debug.Log("Looking at " + hexTile.name);
            ...
            **/
            ...
            hexTile.FindNeighbors();
            //Debug.Log("Neighbors are: " + hexTile.neighbors.ToString());

            for (int j = 0; j < 6; j++)
            {
                if (hexTile.GetNeighbor(j) != null)
                {
                    //Debug.Log("Looking at neighbor " + hexTile.GetNeighbor(j).name);

                    if (j == 1)
                    {
                        Debug.Log("Turning off nw wall");
                        //tile.gameObject.transform.FindChild("NW_Wall").gameObject.SetActive(false);
                        GameObject.Destroy(tile.gameObject.transform.FindChild("NW_Wall").gameObject);
                    }
                    else if (j == 3)
                    {
                        Debug.Log("Turning off sw wall");
                        //tile.gameObject.transform.FindChild("SW_Wall").gameObject.SetActive(false);
                        GameObject.Destroy(tile.gameObject.transform.FindChild("SW_Wall").gameObject);
                    }
                    else if (j == 5)
                    {
                        Debug.Log("Turning off e wall");
                        GameObject.Destroy(tile.gameObject.transform.FindChild("E_Wall").gameObject);
                    }
                }
            }
        }
